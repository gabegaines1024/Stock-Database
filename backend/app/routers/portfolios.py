...
def create_portfolio_route(portfolio: PortfolioCreate, db: Session = Depends(get_db)) -> Portfolio:
    """Create a new portfolio."""
    try:
        return cast(Portfolio, create_portfolio(db, portfolio))
...
def get_portfolio_route(portfolio_id: int, db: Session = Depends(get_db)) -> Portfolio:
    ...
    try:
        return cast(Portfolio, get_portfolio(db, portfolio_id))
    except HTTPException as e:
        raise e
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def list_portfolios_route(db: Session = Depends(get_db)) -> list[Portfolio]:
    """List all portfolios."""
    try:
        return cast(list[Portfolio], list_portfolios(db))
    except HTTPException as e:
        raise e
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
